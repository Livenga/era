...
__kernel void
class_rfcn(__private size_t     width,
  ...
  /* 入出力データのオフセット */
  const size_t in_offset = CanvasSize(width, height) * idx_x;
  /* 遺伝子型の横幅 */
    ...
    for(cnt_unit = 0; cnt_unit < column; cnt_unit++) {
      ...
      if(r_spd[cnt_unit] <= cnt_spd) {
        ...
        for(i = 0; i < InternalSize(gtype[0]); i++)
          r_internal[i] =
            w[(gtype[1 + ParameterSize(gtype[0]) + i] >> position) & 0x0F];

        for(y = 0; y < 5; y++) {
          for(x = 0; x < 5; x++) {
            input_x += r_internal[index[y][x]] *
              ncv_data[(y * width + x) + in_offset];
          }
        }

        offset = 1 + ParameterSize(gtype[0]) +
          InternalSize(gtype[0]) + ExternalSize(gtype[0]);
  ...
  vector[(3 * idx_x) + 0] = unit_output[column - 3];
  vector[(3 * idx_x) + 1] = unit_output[column - 2];
  vector[(3 * idx_x) + 2] = unit_output[column - 1];
